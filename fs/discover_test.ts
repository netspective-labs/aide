...
Deno.test("discover path in ancestors", async () => {
  ...
  // when running in GitHub actions, the root of the repo is aide/aide
  // not netspective-labs/aide
  const result = await mod.discoverGlob(
    isCICD ? "**/aide/aide" : "**/netspective-labs/aide",
    path.dirname(path.fromFileUrl(import.meta.url)),
  );
  ta.assert(result);
  ta.assert(result.found);
  ta.assert(result.found.isDirectory);
  ta.assertEquals(result.found.name, "aide");
